...
class PeopleControllerTestCase(ControllerTestCase):
    ...
    def test_get_person(self):
        id_ = 1
        article_data = self.default_person_json(id_)
        self.make_a_person(id_)

        # get made person
        get_response = self.app.get('/people/{0}'.format(id_))
        self.check_jsonapi_response(get_response, 200, article_data)
    ...
    def test_create_valid_person(self):
        id_ = 1
        article_data = self.default_person_json(id_)
        create_response = self.app.post('/people/{0}'.format(id_), data=json.dumps(article_data),
                                        content_type='application/json')
        self.check_jsonapi_response(create_response, 201, article_data)
    ...
    def test_create_invalid_person(self):
        ...
        create_response = self.app.post('/people/1', data=json.dumps(article_data), content_type='application/json')
    ...
    def test_create_best_effort_personid(self):
        ...
        create_response = self.app.post('/people/1', data=json.dumps(article_data_no_id),
                                        content_type='application/json')
    ...
    def test_delete_person(self):
        ...
        self.check_jsonapi_response(delete_response, 200, article_data)
    ...
    def test_update_person(self):
        ...
        # modify the person
        article_data['name'] = 'John Doe'
        update_response = self.app.put('/people/{0}'.format(id_), data=json.dumps(article_data),
                                       content_type='application/json')
        self.check_jsonapi_response(update_response, 200, article_data)
    ...
    def test_update_invalid_person(self):
        ...
        del article_data['data']['attributes']['name']
        update_response = self.app.put('/people/{0}'.format(id_), data=json.dumps(article_data),
                                       content_type='application/json')
    ...
    def test_update_nonexistant_person(self):
        ...
        id_ = 987
        article_data = self.default_person_json(id_)
        response = self.app.put('/people/{0}'.format(id_), data=json.dumps(article_data), content_type='application/json')
        self.check_response(response, 404,
                            {
                                'error': 'The requested URL was not found on the server.  If you entered the URL manually please check your spelling and try again.'})
